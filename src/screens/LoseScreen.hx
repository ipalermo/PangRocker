...
class LoseScreen extends Screen {
	...
	public function new(sm:ScreenManager) {
		super(sm);
		b1 = new Button(400, 100, 0xFFFFFF, retry, 'Retry');
		b2 = new Button(400, 100, 0xFFFFFF, quit, 'Quit');
	...
	public function retry(evento:MouseEvent) {
		getScreenManager().switchScene('game');
	}

	public function quit(evento:MouseEvent) {
		getScreenManager().switchScene('scores');
	}
